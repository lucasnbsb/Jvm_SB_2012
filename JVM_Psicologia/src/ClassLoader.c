	...
	for (i = 1; i < cf->constant_pool_count; i++) {
		...
		switch (cf->constant_pool[i].tag) {
			...
			case CONSTANT_Methodref:
				...
				cf->constant_pool[i].info.methodRefInfo.nameAndTypeIndex = lerU2 (fp);
				break;

			case CONSTANT_InterfaceMethodRef:
				cf->constant_pool[i].info.interfaceMethodRefInfo.classIndex = lerU2 (fp);
				cf->constant_pool[i].info.interfaceMethodRefInfo.nameAndTypeIndex = lerU2 (fp);
				break;
			...
			case CONSTANT_NameAndType:
				...
				cf->constant_pool[i].info.nameAndTypeInfo.descriptorIndex = lerU2 (fp);
				break;
...
void lerAttributeCode (FILE *fp, ClassFile *cf, attributeInfo *attr){
	...
	attr->tipoInfo.code.code = malloc ((attr->tipoInfo.code.codeLength) * sizeof (u1));
	...
	attr->tipoInfo.code.excTable = malloc ((attr->tipoInfo.code.exceptionTableLength) * sizeof(exceptionTable));
	...
	attr->tipoInfo.code.attributes = malloc ((attr->tipoInfo.code.attributesCount) * sizeof (attributeInfo));
...
void lerAttributeExceptions (FILE *fp, attributeInfo *attr){
	...
	attr->tipoInfo.exceptions.exceptionIndexTable = malloc ((attr->tipoInfo.exceptions.numberOfExceptions) * sizeof(u2));
...
void lerAttributeInnerClasses (FILE *fp, attributeInfo *attr){
	...
	// aloca o espaço necessário
	attr->tipoInfo.innerClasses.classes = malloc ((attr->tipoInfo.innerClasses.numberOfClasses) * sizeof(classes));
	...
	for (i = 0; i < attr->tipoInfo.innerClasses.numberOfClasses; i++){
		...
		attr->tipoInfo.innerClasses.classes[i].innerClassAccessFlags = lerU2(fp);
	}
...
void lerAttributeInfo (FILE *fp, ClassFile *cf, attributeInfo *attr){
	...
	strcpy(nomeAtributo, cf->constant_pool[attr->attributeNameIndex].info.UTF8Info.bytes);
...
void lerFieldInfo (FILE *fp, ClassFile *cf){
	...
	for (i = 0; i < cf->fields_count; i++){
		...
		cf->fields[i].attributes = malloc ((cf->fields->attributesCount) * sizeof(attributeInfo));
...
void lerMetodos (FILE *fp, ClassFile *cf){
	...
	for (i = 0; i < cf->methods_count; i++){
		...
		// aloca o espaço necessário
		cf->methods[i].attributes = malloc ((cf->methods[i].attributesCount) * sizeof (attributeInfo));
